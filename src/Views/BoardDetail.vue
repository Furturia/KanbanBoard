...
const addCol = async () => {
  ...
  if (newColName.value) {
    ...
    if (Array.isArray(board.value.cols) && board.value.cols.length > 0) {
      const validIds = board.value.cols
        .map((col) =>
          typeof col.id === "number" && !isNaN(col.id) ? col.id : null
        )
        .filter((id) => id !== null);
      maxId = validIds.length > 0 ? Math.max(...validIds) : 0;
    }
    const newId = maxId + 1;
    if (!board.value.cols || board.value.cols.length == 0) {
      board.value.cols = [];
    }
...
const addCard = async () => {
  ...
  await updateBoardApi();
  if (addNewCard.members) {
    addNewCard.members.forEach(async (member) => {
      if (member !== user.value.email) {
        await addNoti("card", addNewCard, member);
      }
    });
  }
...
const onUpdateCard = async () => {
  ...
  const oldMembers = targetCard.value.members;
  const newMembers = updateForm.value.members;
  ...
  if (addedMembers) {
    addedMembers.forEach(async (member) => {
      if (member !== user.value.email) {
        await addNoti("card", targetCard.value, member);
      }
    });
  }
  closeAddCardModal();
...
const addMember = () => {
  if (memberEmail.value && memberEmail.value.includes("@")) {
    if (!board.value.members.includes(memberEmail.value)) {
      ...
      addedMembers.value.push(memberEmail.value)
      memberEmail.value = "";
...
const addNoti = async (type, b, email) => {
  try {
    ...
    response.noti
      ? response.noti.push({
          title: `คุณถูกเพิ่มใน${
            type == "board" ? "บอร์ด" : type == "col" ? "คอลัมน์" : "การ์ด"
          }ใหม่`,
          boardId: board.value.id,
          message: `คุณถูกเชิญให้เข้าร่วม ${
              type == "card" ? b.title : b.name
            } โดย ${user.value.email}`,
        })
      : (response.noti = [
          {
            title: `คุณถูกเพิ่มใน${
...
const inviteMember =async ()=>{
  await updateBoardApi();
  if (addedMembers.value) {
    addedMembers.value.forEach(async (member) => {
      if (member !== user.value.email) {
        await addNoti("board", board.value, member);
        addedMembers.value=[]
      }
    });
  }
...
<template>
  ...
  >
    ...
    >
      <form @submit.prevent="inviteMember" class="flex flex-col gap-6">
